...
def run_grid_search(train_user, test_user, train_movie, test_movie, n_users, n_movies):
    print("Starting Grid Search...")
    results_data = []

    for k_, lambda_, tau_ in product(config.K_VALUES, config.LAMBDA_VALUES, config.TAU_VALUES):
        print(f"Testing configuration: k={k_}, lambda={lambda_}, tau={tau_}")
        run_dir = os.path.join(config.SAVE_DIR, f"k={k_}_lambda={lambda_}")
